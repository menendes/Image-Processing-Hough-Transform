...
for r=25:1:26
    ...
    for i=1:size(b,1)
        for j=1:size(b,2)
            if(b(i,j)==1 )
                for t=0:1:360
                    ac(1)=round(i+r*(cos(t*PI/180)));
                    ac(2)=round(j+r*(sin(t*PI/180)));
...
for i=1:1:5
    ...
    for t=1:1:360
        ac(1)=round(circles(i,1)+2*circles(i,3)*cos(t*PI/180));
        ac(2)=round(circles(i,2)+2*circles(i,3)*cos(t*PI/180));
        if(ac(1)>=1 && ac(1)<=246 && ac(2)>=1 && ac(2)<=300)
            c((ac(1)),(ac(2)))=1;
        end
    end
